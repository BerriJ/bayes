...
For the frequentists \ac{LASSO} $\lambda$-parameter we used the __cv.glmnet__ cross-validation function from the __glmnet__ package  and we got a $\lambda$ of `r round(LA,5)` @hastle_glmnet_2019
...
As one can see in Tabel \ref{tab:sum_bay} the _median_ for all regressors are unequal to zero, whereas for the frequentist \ac{LASSO} we have `r sum(glmnet_coef == 0)` coefficient which are directly ecluded from the model, e.g. zero. However, it is unlikely that for multidimensional bayesian model the median for a parameter is zero, since the computation depends on a Gibbs sampler. If we instead look at the 95 \%  credible interval we finde that `r sum(BA[,2] * BA[,3] <= 0)` of these intervall include the zero.  
...
# \acf{RMSE} and 'Sensitive Analysis' 
